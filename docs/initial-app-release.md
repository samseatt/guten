...
Archive the published build record, registry verification report, media inventory and tar alongside the existing database/credential archives. Registry tokens are not included in those artifacts. Application deployment, production OAuth setup and DNS cutover have not yet occurred. Subsequent media installation and release staging are recorded in [app-host bootstrap](cloud-app-bootstrap.md).
